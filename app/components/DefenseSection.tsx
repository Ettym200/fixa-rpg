...
export default function DefenseSection() {
  const { mods } = useSheet();
  const [armor, setArmor] = useLocalStorage<number>("def_armor", 0);
  const [shield, setShield] = useLocalStorage<number>("def_shield", 0);
  const [others, setOthers] = useLocalStorage<number>("def_others", 0);
  const [penalty, setPenalty] = useLocalStorage<number>("def_penalty", 0);
